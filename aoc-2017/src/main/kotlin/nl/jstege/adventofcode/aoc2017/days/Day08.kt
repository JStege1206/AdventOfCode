...
class Day08 : Day() {
    ...
    override fun first(input: Sequence<String>): Any {
        return input
                .parse()
                .fold(mapOf<String, Int>()) { rs, instr -> rs + (instr.r1 to instr(rs)) }
                .values
                .max() ?: 0
    }
    ...
    override fun second(input: Sequence<String>): Any {
        return input
                .parse()
                .fold(mapOf<String, Int>() to Int.MIN_VALUE) { (rs, oldMax), instr ->
                    val result = instr(rs)
                    val max = if (result > oldMax) result else oldMax
                    (rs + (instr.r1 to result)) to max
                }
                .second
    }
